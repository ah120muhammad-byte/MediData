...
class AiSource {
  ...
  const AiSource({required this.title, required this.uri});
}
...
class AiChatService {
  ...
  static const String _publishableKeyFallback =
  ...
  Future<AiChatResponse> sendMessageStreaming({
    ...
    // Advanced tools use the regular endpoint so we can reliably return
    // tool metadata and grounding sources. Normal mode keeps the live stream.
    if (useWebSearch || useCodeExecution) {
      return sendMessage(
        messages: messages,
        mode: mode,
        useWebSearch: useWebSearch,
    ...
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        throw Exception(
          _extractError(
            _decodeJsonResponse(body),
            fallback: 'Unable to get an AI response.',
          ),
        );
      }
      ...
      await for (final line in response.stream
        ...
        try {
          ...
          final chunk = decoded is Map
              ? (decoded['delta'] ?? decoded['content'] ?? decoded['reply'] ?? '')
              : decoded.toString();
  ...
  Future<AiChatResponse> sendMessageWithAttachment({
    ...
    request.fields['useWebSearch'] = useWebSearch.toString();
    request.fields['useCodeExecution'] = useCodeExecution.toString();
    request.files.add(
    ...
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw Exception(
          _extractError(
            data,
            fallback: 'Unable to process the attachment.',
          ),
        );
      }
  ...
  void _validateMessages(List<AiChatMessage> messages) {
    ...
    for (final message in messages) {
      if (message.role.trim().isEmpty || message.content.trim().isEmpty) {
        throw Exception('A message cannot be empty.');
      }

      if (!{'user', 'assistant', 'system'}.contains(message.role.trim())) {
        throw Exception('Invalid AI message role: ${message.role}');
      }
    }
  }
  ...
  void _validateAttachment(AiAttachment attachment) {
    ...
    if (attachment.bytes.length > maxFileBytes) {
      throw Exception('File size must be 20 MB or less.');
    }

    if (!isSupportedFile(attachment.fileName)) {
  ...
  AiChatResponse _parseResponse(dynamic data) {
    ...
    final reply =
        (map['reply'] ?? map['response'] ?? map['message'])?.toString().trim() ?? '';
      ...
      for (final item in rawSources) {
        if (item is Map) {
          ...
          if (title.isNotEmpty && uri.isNotEmpty) {
            sources.add(AiSource(title: title, uri: uri));
          }
        }
      }
    }
  ...
  String _extractError(dynamic data, {required String fallback}) {
    if (data is Map) {
      final value =
          (data['error'] ?? data['message'] ?? data['detail'])?.toString().trim();
  ...
  Exception _normalizeFunctionError(Object error) {
    ...
    if (lower.contains('401') || lower.contains('unauthorized')) {
      return Exception('Your session is not authorized. Please log in again.');
    }
    if (lower.contains('403') || lower.contains('forbidden')) {
      return Exception('You are not allowed to use the AI service.');
    }
    if (lower.contains('timeout') || lower.contains('timed out')) {
      return Exception('The AI service took too long to respond.');
    }
    ...
    if (lower.contains('provider')) {
      return Exception('The AI provider is temporarily unavailable.');
    }
