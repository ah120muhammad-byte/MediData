...
class _AppShellState extends State<AppShell> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      try {
        ...
        debugPrint('AppShell notification initialization error: $e');
      }
  ...
  Future<void> _openLectureFromNotification(String lectureId) async {
    ...
    try {
      ...
      if (response == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('This lecture is no longer available.')),
        );
      ...
      if (moduleId == null || moduleId.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Unable to find this lecture module.')),
        );
      ...
      debugPrint('Open lecture error: $e');
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Unable to open this lecture.')),
      );
    }
  }
  ...
  void _openExamHistory() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => ExamHistoryScreen(onAttemptTap: _openExamAttempt),
      ),
    );
  }
  ...
  Future<void> _openExamAttempt(StudentExamAttempt attempt) async {
    ...
    try {
      ...
      if (response == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Exam information is no longer available.'),
          ),
        );
      ...
      debugPrint('Open exam attempt error: $e');
  ...
  void _openSearch() {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) =>
            SearchScreen(onOpenLecture: _openLectureFromNotification),
      ),
    );
  }
}
...
class _AppHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final headerRadius = Responsive.clamped(
      context,
      base: 20,
      min: 16,
      max: 28,
          ...
          children: [
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                      ...
                      offset: Responsive.clampedOffset(
                        context,
                        base: const Offset(0, 6),
                        minScale: 0.8,
                        maxScale: 1.25,
                      ),
                    ),
            ...
            Container(
              ...
              padding: EdgeInsets.all(
                Responsive.clamped(context, base: 5, min: 4, max: 7),
              ),
              decoration: BoxDecoration(
                color: theme.colorScheme.surface,
...
class _CustomBottomNavigation extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final borderRadius = Responsive.clamped(
      context,
      base: 26,
      min: 22,
      max: 34,
    ...
    final maxWidth = width >= 900 ? 720.0 : double.infinity;
...
class _AnimatedNavItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final iconSize = Responsive.clamped(context, base: 25, min: 22, max: 30);

    final normalIconSize = Responsive.clamped(
      context,
      base: 23,
      min: 20,
      max: 28,
          ...
          children: [
            ...
            AnimatedPadding(
              ...
              padding: EdgeInsets.only(
                top: selected
                    ? Responsive.clamped(context, base: 17, min: 14, max: 22)
                    : Responsive.clamped(context, base: 2, min: 0, max: 5),
              ),
            ...
            AnimatedPositioned(
              ...
              curve: Curves.easeOut,
              bottom: selected
                  ? Responsive.clamped(context, base: 8, min: 5, max: 12)
                  : -10,
              child: AnimatedOpacity(
                duration: const Duration(milliseconds: 220),
