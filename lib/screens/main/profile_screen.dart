...
class ProfileScreen extends StatefulWidget {
  ...
  const ProfileScreen({super.key, this.onOpenLecture, this.onExamAttemptTap});
...
class _Avatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (url != null && url.isNotEmpty) {
      return CircleAvatar(radius: 34, backgroundImage: NetworkImage(url));
    }
...
class _OverviewGrid extends StatelessWidget {
  ...
  const _OverviewGrid({required this.analytics, required this.isTablet});
...
class _StudyActivityChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final spots = List.generate(
      activity.length,
      (index) => FlSpot(index.toDouble(), activity[index].studyMinutes),
    );

    final maxY = activity
        .map((item) => item.studyMinutes)
  ...
  Widget build(BuildContext context) {
    ...
    if (completed.isEmpty) {
      return const SizedBox(
        height: 180,
        child: Center(child: Text('No completed exams yet.')),
      );
    }
