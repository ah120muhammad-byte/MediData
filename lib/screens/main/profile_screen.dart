...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _showEditProfile(StudentProfile profile) async {
    final saved = await showDialog<bool>(
      context: context,
      builder: (_) => _EditProfileDialog(profile: profile, service: _service),
    );
...
class _EditProfileDialogState extends State<_EditProfileDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      final emailChanged =
          email != widget.profile.email.trim().toLowerCase();

      if (emailChanged) {
        await widget.service.updateEmail(email);
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Unable to update your profile. Please check the email and try again.',
          ),
        ),
      );
    }
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  helperText: 'A confirmation email may be sent to the new address.',
