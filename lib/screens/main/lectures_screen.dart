...
class _LecturesScreenState extends State<LecturesScreen> {
  ...
  void _toggleLecture(String lectureId) {
    if (!mounted) return;
    setState(() {
      _expandedLectureId = _expandedLectureId == lectureId ? null : lectureId;
    });
  }
          ...
          content: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  color: dialogTheme.colorScheme.onSurface.withValues(alpha: 0.70),
                ),
              ),
    ...
    try {
      ...
      final downloaded = await downloadsService.download(
        ...
        onProgress: (progress) {
          if (mounted) {
            debugPrint(
              'Downloading ${file.title}: ${(progress.progress * 100).round()}%',
            );
          }
        },
      );
...
class _LectureFile {
  ...
  factory _LectureFile.fromMap(Map<String, dynamic> map) => _LectureFile(
        id: map['id']?.toString() ?? '',
